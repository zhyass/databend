    ...
    async fn execute2(&self) -> Result<PipelineBuildResult> {
            ...
            .get_enable_compact_after_multi_table_insert()?
        {
            for (_, (db, tbl, branch)) in &self.plan.target_tables {
                if branch.is_some() {
                    // currently, we only support compact for main
                    continue;
                }
...
impl InsertIntoBranches {
    ...
    async fn build_serializable_target_tables(
        ...
        for table in &self.tables {
            ...
            let branch = table.get_table_branch().map(|b| b.name.clone());
            serializable_tables.push(SerializableTable {
                target_catalog_info: catalog_info,
                target_table_info: ResolvedTableInfo::new(table_info).with_branch(branch),
    ...
    async fn build_deduplicated_serializable_target_tables(
        ...
        for table in &self.tables {
            let branch_info = table.get_table_branch();
            let tid = branch_info
                .as_ref()
                .map_or(table.get_id(), |v| v.branch_id());
            if last_table_id == Some(tid) {
                continue;
            }
            last_table_id = Some(tid);
            let table_info = table.get_table_info();
            let catalog_info = ctx.get_catalog(table_info.catalog()).await?.info();
            let fuse_table = FuseTable::try_from_table(table.as_ref())?;
            let snapshot = fuse_table.read_table_snapshot().await?;
            ...
            serializable_tables.push(SerializableTable {
                target_catalog_info: catalog_info,
                target_table_info: ResolvedTableInfo::new(table_info)
                    .with_branch(branch_info.map(|b| b.name.clone())),
                table_meta_timestamps,
            });
        }
        Ok(serializable_tables)
    ...
    fn build_group_ids(&self) -> Vec<u64> {
        self.tables
            .iter()
            .map(|table| table.table_or_branch_id())
            .collect()
    }
}
