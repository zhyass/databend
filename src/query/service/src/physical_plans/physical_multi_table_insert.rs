        ...
        for fill_and_reorder in &self.fill_and_reorders {
            if let Some(fill_and_reorder) = fill_and_reorder {
                ...
                if fill_and_reorder.source_schema != default_schema {
                    ...
                    if let Some((async_funcs, new_schema, new_schema_no_cast)) = binder
                        .split_async_default_exprs(
                        ...
                        )?
                    {
                        ...
                        if new_schema != new_schema_no_cast {
                            plan.cast_builder =
                                Some(Box::new(builder.cast_schema_transform_builder(
                                    new_schema_no_cast.clone(),
                                    new_schema.clone(),
                                )?));
                        }
                        ...
                        plan.resort_builder = Some(Box::new(move |input, output| {
