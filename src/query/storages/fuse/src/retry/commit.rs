...
async fn compute_table_segments_diffs(
    ...
    HashMap<u64, HashMap<u64, SegmentsDiff>>,
    HashMap<u64, HashMap<u64, Option<Arc<TableSnapshot>>>>,
)> {
...
async fn try_rebuild_req(
    ...
    table_segments_diffs: &HashMap<u64, HashMap<u64, SegmentsDiff>>,
    table_original_snapshots: &HashMap<u64, HashMap<u64, Option<Arc<TableSnapshot>>>>,
) -> Result<()> {
    info!(
    ...
    for (tid, seq, table_meta) in update_failed_tbls {
        ...
        let schema = latest_table.schema();
        ...
        for (branch_id, base_location) in base_snapshot_locations {
            ...
            let table_branch_id = if branch_id == 0 { tid } else { branch_id };
            let insert_row = insert_rows.get(&table_branch_id).cloned().unwrap_or(0);
...
fn get_branch_snapshot_location(table: &FuseTable, branch_id: u64) -> Result<Option<String>> {
    if branch_id == 0 {
        ...
        let branch = table.get_table_info().get_branch_info_by_id(branch_id)?;
        Ok(Some(branch.info.loc))
    }
}
