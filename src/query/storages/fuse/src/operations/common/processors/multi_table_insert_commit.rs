...
impl AsyncSink for CommitMultiTableInsert {
    ...
    async fn on_finish(&mut self) -> Result<()> {
        ...
        for (tid, commit_meta) in std::mem::take(&mut self.commit_metas).into_iter() {
            ...
            let table_meta_timestamps = self.table_meta_timestamps.get(&tid).unwrap();
    ...
    async fn consume(&mut self, data_block: DataBlock) -> Result<bool> {
        ...
        match self.commit_metas.get_mut(&meta.table_target_id) {
            Some(m) => {
                let table = self.tables.get(&meta.table_target_id).unwrap();
            ...
            None => {
                self.commit_metas.insert(meta.table_target_id, meta);
            }
        }
        Ok(false)
    }
