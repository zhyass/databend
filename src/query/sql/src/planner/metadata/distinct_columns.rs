...
impl FromStr for DistinctColumns {
    type Err = ErrorCode;

    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        let s = s.trim();
        if s.is_empty() {
            return Ok(DistinctColumns::None);
        }

        if s.to_lowercase() == "all" {
            return Ok(DistinctColumns::All);
        }
        ...
        Ok(DistinctColumns::Specify(cols))
    }
}

impl DistinctColumns {
        ...
        F: Fn(&TableDataType) -> bool,
    {
        ...
        if definition.to_lowercase() == "all" {
            return Ok(());
        }
