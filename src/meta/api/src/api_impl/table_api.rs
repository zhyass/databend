...
async fn build_create_table_clone_txn(
    ...
    let source_group_id = parse_clone_group_id(&source_meta.options)
        .map_err(|err| {
            KVAppError::AppError(
    ...
    let clone_group_id = clone.clone_group_id.to_string();
    if source_group_id != clone.clone_group_id
        || req.table_meta.options.get(OPT_KEY_CLONE_GROUP_ID) != Some(&clone_group_id)
    {
    ...
    let mask_policy_ids = req
        ...
        .collect::<HashSet<_>>();
    if !mask_policy_ids.is_empty() {
        let mask_policy_idents = mask_policy_ids.into_iter().map(|policy_id| {
            DataMaskIdIdent::new_generic(req.tenant(), DataMaskId::new(policy_id))
        });
        for (policy_ident, policy) in kv_api.get_pb_vec(mask_policy_idents).await? {
            let policy_id = *policy_ident.data_mask_id();
    ...
    if let Some(policy) = &req.table_meta.row_access_policy_columns_ids {
        ...
        let policy_ident =
            RowAccessPolicyIdIdent::new_generic(req.tenant(), RowAccessPolicyId::new(policy_id));
        let policy = kv_api.get_pb(&policy_ident).await?.ok_or_else(|| {
            KVAppError::AppError(
                InvalidMaterializedView::new(format!(
...
mod tests {
    ...
    use super::validate_create_table_request;

    #[test]
    fn test_clone_create_rejects_source_option_update() {
        let tenant = Tenant::new_literal("tenant");
        let now = Utc::now();
        let req = CreateTableReq {
            create_option: CreateOption::Create,
            catalog_name: None,
            name_ident: TableNameIdent::new(&tenant, "db", "clone"),
            table_meta: TableMeta {
                engine: "FUSE".to_string(),
                drop_on: Some(now),
                ..Default::default()
            },
            source_table_option: Some(UpsertTableOptionReq {
                table_id: 2,
            ...
            clone: Some(CreateTableCloneMeta {
                source_table_id: 1,
                source_table_seq: MatchSeq::Exact(1),
                clone_group_id: 1,
        ...
        assert!(validate_create_table_request(&req).is_err());
    }
